  ...
  return (
    <div className={styles.dashboard}>
      ...
      <div className={styles.card}>
        <div className={styles.cardTitleRow}>
          <span className={styles.cardTitle}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{verticalAlign:'middle',marginRight:'6px'}}><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>
            항목별 소모품 잔여 수명
          </span>
        </div>
        {!(carYear || carMileage) ? (
          <div className={styles.consumablesEmpty}>
            차량 연식과 주행거리를 등록하시면<br />실시간 교체 주기 및 권장 상태 진단이 표시됩니다.
          </div>
        ) : (
              ...
              return (
                <div key={idx} className={`${styles.consumableItem} ${isDanger ? styles.itemDanger : isWarning ? styles.itemWarning : styles.itemGood}`}>
                  ...
                  <div className={styles.consumableSubBadgeRow}>
                    ...
                    <button type="button" className={styles.diagBtnReg}
                      onClick={() => {
                        if (onNext) onNext(item.name)
                      }}
                      title="정비 내역 입력 화면으로 이동">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{verticalAlign:'middle',marginRight:'3px'}}><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>
                      등록
                    </button>
                    {hasHistory && (
                      <button type="button" className={styles.diagBtnHistory}
                        onClick={() => { setSelectedDiagItem(item.name); setIsHistoryModalOpen(true) }}
                        title="과거 정비 이력 보기">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{verticalAlign:'middle',marginRight:'3px'}}><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
