...
function BrandLogo({ maker, isGuest }) {
  if (isGuest || (maker && (maker.includes('테슬라') || maker.includes('TESLA')))) {
    // Icons8 스타일 테슬라 시그니처 쉴드 엠블럼 SVG
    return (
      <svg
        role="img"
        viewBox="0 0 50 50"
      ...
      >
        <path
          d="M25 5C17.5 5 11 6.8 6 9.8L8.2 13.8C12.5 11.2 18.2 9.8 25 9.8C31.8 9.8 37.5 11.2 41.8 13.8L44 9.8C39 6.8 32.5 5 25 5Z"
          fill="#e82127"
        />
        <path
          d="M25 12.5C20.5 12.5 16.2 13.4 12.8 15L14.2 18C17 16.7 20.7 16 25 16C29.3 16 33 16.7 35.8 18L37.2 15C33.8 13.4 29.5 12.5 25 12.5Z"
          fill="#e82127"
        />
        <path
          d="M23.5 17.5V45H26.5V17.5H23.5Z"
          fill="#e82127"
        />
      </svg>
    )
...
export default function Dashboard({
  ...
  const getUserPhotoKey = (user) => {
    if (!user || !user.email) return 'carrep_user_car_photo'
    const idKey = user.email.split('@')[0].replace(/[^a-zA-Z0-9_-]/g, '_')
    return `carrep_user_car_photo_${idKey}`
  }
  ...
  useEffect(() => {
    try {
      ...
      const savedPhoto = localStorage.getItem(photoKey) || localStorage.getItem('carrep_user_car_photo')
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        removeBgFromImage(rawBase64, (processedPng) => {
          ...
          const photoKey = getUserPhotoKey(currentUser)
          localStorage.setItem(photoKey, processedPng)
          ...
          <div className={styles.profileCardTopRow}>
            <div className={styles.carNicknameRow}>
              <span className={styles.carNickname}>
                {currentUser ? (carNickname || carName || '내 차량') : '마이카'}
              </span>
          ...
          <div className={styles.profileCardBottomRow}>
            <div className={styles.profileBottomLeft}>
              ...
              {currentUser && (carDriveType || carGrade || carMileage) && (
                isMobile ? (
                  <div className={styles.carSpecBadgesMobileRow}>
                    {(carDriveType || carGrade) && (
                      <span className={styles.mobileSpecChip}>
                        {[carDriveType, carGrade].filter(Boolean).join(' · ')}
                      </span>
                    )}
                    {carMileage && (
                      <span className={styles.mobileSpecChip}>
                        {Number(carMileage).toLocaleString()}Km
                      </span>
                    )}
                  </div>
                ) : (
          ...
          {inspection?.startDate ? (
            <>
              <div className={styles.insuranceRow}>
                ...
                <div style={{ flex: 1 }}>
                  <div className={styles.insuranceName}>정기 검사 보증</div>
                  <div className={styles.insuranceDate}>
