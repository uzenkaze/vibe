...
export function AppProvider({ children }) {
  ...
  const [navSection, setNavSection] = useState(() => {
    ...
    if (nav && ['dashboard', 'assets', 'expenses', 'installment', 'cardPayments', 'pension', 'insurance'].includes(nav)) {
      return nav;
    }
    return 'dashboard';
  });
  ...
  const getCurrentPension = useCallback(() => {
    const yd = yearData[year] || {};
    const months = yd.months || {};
    if (months[month]?.pension) return months[month].pension;

    // 현재 월에 pension 데이터가 없는 경우, 최신 월의 pension 데이터 탐색
    const monthKeys = Object.keys(months).sort();
    for (let i = monthKeys.length - 1; i >= 0; i--) {
      const m = monthKeys[i];
      if (months[m]?.pension) return months[m].pension;
    }

    return { beforeTax: '', afterTax: '', startDate: '', totalPaid: '', totalPeriod: '' };
  }, [yearData, year, month]);

  const getCurrentInsurance = useCallback(() => {
    // 보험 데이터는 연도 수준 공통 데이터 (특정 월에 종속되지 않음)
    const yd = yearData[year] || {};
    if (yd.insurance) return yd.insurance;
    // 다른 연도에서 탐색
    const yearKeys = Object.keys(yearData).sort().reverse();
    for (const y of yearKeys) {
      if (yearData[y]?.insurance) return yearData[y].insurance;
    }
    return { policies: [] };
  }, [yearData, year]);
  ...
  const loadYearData = useCallback(async (targetYear) => {
    try {
      ...
      if (data) {
        // [자동 데이터 마이그레이션] 부동산(real-estate) 내역이 존재하는 경우 비현금성 자산(non-cash)으로 통합 이전
        if (data.months) {
          let hasMigration = false;
          Object.keys(data.months).forEach(mKey => {
            const sec = data.months[mKey]?.sections;
            if (sec && Array.isArray(sec['real-estate']) && sec['real-estate'].length > 0) {
              if (!Array.isArray(sec['non-cash'])) sec['non-cash'] = [];
              
              // 중복 방지 및 이전
              sec['real-estate'].forEach(reItem => {
                const exists = sec['non-cash'].some(nc => nc.id === reItem.id || (nc.content === reItem.content && nc.amount === reItem.amount));
                if (!exists) {
                  sec['non-cash'].push({
                    ...reItem,
                    category: reItem.category || '부동산'
                  });
                }
              });
              sec['real-estate'] = [];
          ...
          if (hasMigration) {
            console.log(`[Migration] Transferred real-estate assets to non-cash for ${targetYear}`);
          }
        }
  ...
  const persistPension = useCallback(async (pensionData) => {
    ...
    // 연금 데이터는 전 월(01~12) 공통 데이터이므로 모든 월에 일괄 동기화
    Object.keys(yd.months).forEach(mKey => {
      if (yd.months[mKey]) {
        yd.months[mKey].pension = pensionData;
      }
    });
    if (!yd.months[month]) yd.months[month] = { pension: pensionData };
  ...
  const persistInsurance = useCallback(async (insuranceData) => {
    const yd = JSON.parse(JSON.stringify(yearData[year] || { year, months: {} }));
    yd.year = year;
    yd.insurance = insuranceData;
    yd.updatedAt = Date.now();
  ...
  return (
    <AppContext.Provider value={{
      ...
      getCurrentSections, getCurrentPension, getCurrentInsurance,
      loadYearData,
      persistSections, persistPension, persistInsurance, persistYearData,
