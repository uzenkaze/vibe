...
const CHANNELS = [
  ...
  { id: 'mbc_m', name: 'MBC M', network: 'MBC', category: '방송/오락', officialUrl: 'https://m.mbcplus.com/web/onair.do?categoryid=4', ytHandle: '@ALLTHEKPOP', ytChannelId: 'UCPde4guD9yFBRzkxk2PatoA', urls: [
  ...
  { id: 'ktv', name: 'KTV 국민방송', network: 'KTV', category: '뉴스/경제', ytHandle: '@KTVKorea', ytChannelId: 'UCj8Snyrs1y-wnBQiUmGrTjw', noPlayableHls: true, urls: [
    'https://hlive.ktv.go.kr/live/klive_h.stream/playlist.m3u8',
    'https://hlive.ktv.go.kr/live/klive_l.stream/playlist.m3u8'
  ]},
  { id: 'arirang', name: 'Arirang TV', network: 'ARIRANG', category: '뉴스/경제', ytHandle: '@arirangnews', ytChannelId: 'UCV2R_C_c4Xp8k-L1qT7_hSw', noPlayableHls: true, urls: [
    'https://amdlive-ch01.ctnd.com.edgesuite.net/arirang_1ch/smil:arirang_1ch.smil/playlist.m3u8'
  ]},
  { id: 'tbs', name: 'TBS 교통방송', network: 'TBS', category: '뉴스/경제', ytHandle: '@tbsnews', ytChannelId: 'UCF_yO86W0-S06z1L299OqYQ', noPlayableHls: true, urls: [
...
async function playChannel(ch, urlIdx = 0, startTime = 0) {
  ...
  // noPlayableHls 플래그가 있거나 재생 가능한 HLS URL이 없으면 즉시 YouTube 라이브 플레이어로 직행
  if (ch.noPlayableHls && (ch.ytChannelId || ch.ytVideoId || ch.ytHandle)) {
  ...
  if (Hls.isSupported()) {
    ...
    hls.on(Hls.Events.ERROR, (e, data) => { 
      ...
      // 비치명적(non-fatal) 조각 로딩 실패 시 자동 복구 시도 (타임아웃으로 잘리지 않도록 보호)
      if (data.type === Hls.ErrorTypes.NETWORK_ERROR && !data.fatal) {
        nonFatalNetworkErrorCount++;
        if (nonFatalNetworkErrorCount >= 10) {
          console.warn('[HLS.js] Non-fatal network errors exceeded threshold (10). Forcing proxy failover...');
      ...
      if (data.fatal) {
        if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
          ...
          if (mediaErrorRetries < 3) {
            mediaErrorRetries++;
            debugLog(`미디어 재생 오류 복구 시도 중... (${mediaErrorRetries}/3)`);
  ...
  // 12초 타임아웃: 버퍼링 대기 시간 보장 (재생 중에는 타임아웃 취소)
...
window.openNetflix = async function() {
  ...
  // 3순위: 모바일 브라우저 환경 (앱 미설치 시 Intent fallback)
  if (isAndroid) {
