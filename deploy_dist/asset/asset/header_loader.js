    ...
    try {
        ...
        if (cloudMenu) {
            if (isConnected) {
                ...
                cloudMenu.innerHTML = `?ï¸ <span style="color: ${greenColor} !important;">Cloud</span>`;
...
function updateThemeUI(isDark) {
    ...
    if (btn) btn.innerText = isDark ? '?€ï¸? : '?Œ™';
}
...
window.addEventListener('beforeunload', (e) => {
    if (autoSyncTimeout || window.pendingSyncPromise) {
        e.preventDefault();
        e.returnValue = '?°ì´?°ê? ?„ì§ GitHub???™ê¸°?”ë˜ì§€ ?Šì•˜?µë‹ˆ?? ?˜ì´ì§€ë¥?ë²—ì–´?˜ì‹œê² ìŠµ?ˆê¹Œ?';
...
function showAutoSyncToast() {
    ...
    toast.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg> ë¡œì»¬ ë°?GitHub ?œë²„ ?™ê¸°???„ë£Œ`;
